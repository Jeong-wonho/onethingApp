...
const DateSelector: React.FC<DateSelectorProps> = ({
  ...
  const handleScroll = (event: NativeSyntheticEvent<NativeScrollEvent>) => {
    ...
    if (
      ...
      contentSize.width - paddingToEdge
    ) {
      setIsScrolling(true);
  ...
  return (
    ...
    >
      {dateList.map((item, index) => (
        <TouchableOpacity
          key={`${item.date}-${index}`}
          style={[styles.dateItem, item.isSelected && styles.selectedDateItem]}
          onPress={() => handleDateSelect(item.date)}
        >
          <Text
            style={[styles.dayName, item.isSelected && styles.selectedText]}
          >
            {item.dayName}
          </Text>
          <Text
            style={[styles.dayNumber, item.isSelected && styles.selectedText]}
          >
            {item.dayNumber}
          </Text>
        </TouchableOpacity>
